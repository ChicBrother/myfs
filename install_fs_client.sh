...
install_path=/fsclient/
package_download_url=https://raw.githubusercontent.com/ChicBrother/myfs/master/FinalSpeed_Client_CLI.zip
package_save_name=FinalSpeed_Client_CLI.zip
bash_download_url=https://raw.githubusercontent.com/ChicBrother/myfs/master/finalspeed_client_bash.zip
bash_save_name=finalspeed_client_bash.zip
...
function install_finalspeed(){
	...
	if ! wget -O $package_save_name $package_download_url ; then
		echo "Download client software failed!"
		exit 1
	fi

	if ! wget -O $bash_save_name $bash_download_url ; then
		echo "Download client bash failed!"
		exit 1
	fi
	...
	unzip -o $bash_save_name  -d $install_path
	
	sh ${install_path}"restart.sh"
	tail -f ${install_path}"client.log"
}
